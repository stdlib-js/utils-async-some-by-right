...
function factory( options, predicate ) {
	...
	if ( !isFunction( f ) ) {
		throw new TypeError( format( '1Rf3q,JV', f ) );
	}
	if ( opts.series ) {
		opts.limit = 1;
	} else if ( !opts.limit ) {
	...
	*/
	function someByRightAsync( collection, n, done ) {
		if ( !isCollection( collection ) ) {
			throw new TypeError( format( '1RfAh,O3', collection ) );
		}
		if ( !isPositiveInteger( n ) ) {
			throw new TypeError( format( '1Rf45,IB', n ) );
		}
		if ( !isFunction( done ) ) {
			throw new TypeError( format( '1Rf3q,JV', done ) );
		}
